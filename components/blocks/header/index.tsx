      ...
      <div className="container">
        <nav className="hidden justify-between lg:flex">
          ...
          <div className="shrink-0 flex gap-2 items-center">
            {header.showLocale && <LocaleToggle />}
            {header.showTheme && <ThemeToggle />}

            {header.buttons?.map((item, i) => {
              return (
                <Button key={i} variant={item.variant}>
                  <Link href={item.url as any} target={item.target || ''} className="flex items-center gap-1 cursor-pointer">
                    {item.title}
                ...
                <div className="mb-8 mt-8 flex flex-col gap-4">
                  <Accordion type="single" collapsible className="w-full">
                    {header.nav?.items?.map((item, i) => {
                      ...
                      return (
                        <Link key={i} href={item.url as any} target={item.target} className="font-semibold my-4 flex items-center gap-2 px-4">
                          {item.icon && <Icon name={item.icon} className="size-4 shrink-0" />}
                          {item.title}
                        </Link>
                      );
